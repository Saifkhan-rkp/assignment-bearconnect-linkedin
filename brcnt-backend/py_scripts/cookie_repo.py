...
import pickle
import time
import requests.cookies as cookielib
from typing import Optional
from py_scripts.settings import COOKIE_PATH
...
class CookieRepository(object):
    ...
    def get(self, username: str) -> Optional[cookielib.RequestsCookieJar]:
    ...
    def _load_cookies_from_cache(self, username: str) -> Optional[cookielib.RequestsCookieJar]:
        cookiejar_filepath = self._get_cookies_filepath(username)
        try:
    ...
    @staticmethod
    def _is_token_still_valid(cookiejar: cookielib.RequestsCookieJar):
        _now = time.time()
